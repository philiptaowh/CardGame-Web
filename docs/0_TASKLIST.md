...
### v2.2.1.6 — char_7 技能 2/3 逻辑修复（🆕 待启动，2026-06-19）

> **背景**：游玩中发现 char_7「衰弱」「虚弱」两个技能的判定顺序错。
> 当前实现：先 `addMarkWithReplacement(target, '弱化', 2)` 再 `target.marks.some(...)`，
> 由于刚加的印记永远存在，导致条件分支**永远为 true** — 每次用技能 2/3
> 都触发额外效果，与设计意图相反。

- [ ] **P1 gameEngine.ts char_7 技能 2** — 改为"先判定后分支"：目标无弱化则加弱化 2 回合；有则为自己加治疗 2 回合
- [ ] **P2 gameEngine.ts char_7 技能 3** — 改为"先判定后分支"：目标无弱化则加弱化 2 回合；有则挂失明 + 失神 1 回合
- [ ] **P3 characters.ts 同步文案** — 技能 2/3 的 description 改为"若目标没有...则...；若目标已有...则..."
- [ ] **P4 卡牌游戏原型.md 同步** — char_7 技能 2/3 描述同步
- [ ] **P5 卡牌游戏2.0原型.md 同步** — char_7 技能 2/3 描述同步（如有）
- [ ] **P6 build 验证** — `npm run build` + `npx tsc --noEmit` 零错误
- [ ] **P7 同步 New_Card_Game_Web + commit + push** — 双端

**AC（验收）**：
- 修复后，对没有弱化的目标用技能 2：只挂弱化，不治疗
- 修复后，对已有弱化的目标用技能 2：不重复挂弱化，但治疗
- 修复后，对没有弱化的目标用技能 3：只挂弱化，不挂失明/失神
- 修复后，对已有弱化的目标用技能 3：不重复挂弱化，但挂失明/失神
- 文档描述与代码逻辑一致（无歧义）
- `npm run build` 零错误
- 双端 HEAD 一致

**关键决策**：
- 判定先于副作用（5_AGENT_RULES §1）：任何"若 X 则 Y"型条件都应 X 判定后再修改状态
